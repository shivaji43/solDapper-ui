...
export function UpgradeBanner({
  ...
  const iconVariants: Variants = {
    ...
    visible: (custom: { x: number; y: number }) => ({
      ...
      rotate: 360,
      transition: {
        x: { duration: 0.3, ease: [0.42, 0, 0.58, 1] as const },
        y: { duration: 0.3, ease: [0.42, 0, 0.58, 1] as const },
    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-white" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div className="relative flex h-[35px] items-center gap-1 rounded-[6px] border   pl-2.5 pr-1 text-sm  ">
            <button
              className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-[#002359]  decoration-[#CAE7FF]  outline-none hover:text-[#f6f7fa] hover:decoration-[#94CCFF] focus-visible:!shadow-[#008FFF] dark:text-[#EAF5FF] dark:decoration-[#003674] dark:hover:text-[#e98f2e] dark:hover:decoration-[#00408A]"
            ...
            <span className="text-[0.8125rem] text-[#005FF2] dark:text-[#c4c7cb]">
              {description}
            </span>
            {onClose && (
              <button 
